...
class Validator
{
    ...
    public static function validate($data, $roles){
        ...
        foreach ($data as $key => $value) {
            // $value == Narek , narekmel1995@gmail.com , 123456  , 123456
                ...
                foreach ($role_row as $item) {
                    if ($item == 'required') {
                        if (self::requiredCheck($value) == false) {
                            self::$errors[] = 'The field ' . $key . ' is required';
                        }
                    }
                    ...
                    if ($item == 'email') {
                        if(strlen($value) > 0){
                            if (self::emailCheck($value) == false) {
                                self::$errors[] = 'Your ' . $key . ' is not valid';
                            }
                        }else{
                            self::$errors[] = 'The field ' . $key . ' is required';
                        }
                    ...
                    if (substr($item, 0, 3) == 'min') {
                        ...
                        if (self::checkMinLength($value, $min_lenght_role[1]) == false) {
                            self::$errors[] = 'Your ' . $key . ' field length must be minimum ' . $min_lenght_role[1];
                        }
                    }
                    ...
                    if (substr($item, 0, 5) == 'equal') {
                        ...
                        if (self::equalCondition($value, $data[$equal_condition[1]]) == false) {
                            self::$errors[] = 'Your ' . $equal_condition[1] . ' and ' . $key . ' fields are not match !';
                        }
                    }
